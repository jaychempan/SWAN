...
def _resnet(args, arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        #                                       progress=progress)
        resnet_ckpt_name = args.resnet_ckpt.split('/')[-1]
        # pretrained on AID
        if resnet_ckpt_name.split('-')[0] == 'aid_28':
            state_dict = torch.load(args.resnet_ckpt, map_location='cuda:{}'.format(args.gpuid))['model']
        # pretrained on ImageNet
        elif resnet_ckpt_name.split('-')[0] == 'resnet50':
            state_dict = torch.load(args.resnet_ckpt)
        else:
